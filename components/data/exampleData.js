const exampleData = {
'bar':
`Bar (ticks on x axis),Leden (bar height),Únor (bar height),Březen (bar height),Duben (bar height),Květen (bar height),Červen (bar height)
Bar který neexistuje,7,0,1,21,5,6
Super Panda Circus,4,8,6,2,12,7
Bar Naproti,7,19,11,8,16,12
Low Cost BAR,20,14,15,29,7,17
Livingstone Club,4,5,6,7,8,9`,

'pie':
`Pie type (labels), Amount eaten (values)
Apple,6
Pumpkin,2
Plum,5
Poppy,25
Tvarohový,4`,

'line':
`km walked (x axis), Elevation 1 [m] (y axis), Elevation 2 [m] (y axis)
1,712,609
2,741,620
3,790,611
5,763,648
6,804,692
7,729,734
11,847,754
12,812,797
13,785,811
14,711,809
15,677,807
16,654,812
17,637,799
18,644,743
19,709,731
20,784,727
21,835,711
22,849,698
26,615,813
27,572,821
28,547,789
29,540,770
30,569,741`,

'scatter':
`name (labels),economy [l/100km],displacement [cm3],horse power,weight [kg],0-100 km/h [s], type (shape)
Audi,21.7,360,175,1733,11,sedan
Bentley,15.7,390,190,1612,8.5,coupe
Citroën,13.8,304,150,1598,11.5,hatchback
Dacia,11.5,232,90,1431,18.2,minivan
Ford,13,258,120,1525,15.1,truck
Honda,10.2,151,95,1308,20.5,sedan
Infinity,12.4,232,90,1442,17.2,SUV
Jaguar,24.3,151,90,1269,20.1,coupe
Kaipan,9.7,232,100,1179,15,hatchback
Lada,15.7,232,100,1211,13,sedan
Mitsubishi,11.8,232,100,1457,16,SUV
Nissan,11.2,199,90,1251,15,convertible
Opel,13,258,110,1621,13.5,sedan
Peugeot,13.1,199,97,1381,15.5,minivan
Renault,16,211,120,1620,14,hatchback
Škoda,14.7,190,100,1450,13.2,sedan
Tesla,15.8,305,150,1123,9.5,supercar
Volksvagen,10,240,110,1411,13,sedan`,
}

export default exampleData;
